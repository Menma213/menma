        ...
        try {
            switch (effect.type) {
                ...
                case 'status': {
                    ...
                    const existingStatusIndex = (baseTarget.activeEffects || []).findIndex(
                    ...
                    if (existingStatusIndex !== -1 && !canStack) {
                        ...
                        baseTarget.activeEffects[existingStatusIndex].duration = effect.duration || 1;
                        result.specialEffects.push(`Refreshed ${effect.status} duration on ${baseTarget.name}`);
                        break;
                    }
                    ...
                    if (effectHandlers.status(effect.chance ?? 100)) {
                        if (existingStatusIndex !== -1 && canStack) {
                            // Stack the effect
                            baseTarget.activeEffects.push(stored);
                            result.specialEffects.push(`Applied additional ${stored.status} to ${baseTarget.name} (stacking)`);
                        } else {
                            // New effect or replacing existing
                            if (existingStatusIndex !== -1) {
                                baseTarget.activeEffects.splice(existingStatusIndex, 1);
                            }
                            baseTarget.activeEffects.push(stored);
                            result.specialEffects.push(`Applied ${stored.status} to ${baseTarget.name} for ${stored.duration} turns`);
                        }
                    } else {
                        result.specialEffects.push(`${baseTarget.name} resisted ${effect.status}`);
                    }
                    break;
                }
...
function applyDamageWithReflection(attacker, defender, damageAmount, actionResult) {
    ...
    if (reflectEffectIndex !== -1) {
        // Reflection active: defender takes no damage, attacker takes damage
        actionResult.specialEffects.push(`${defender.name}'s Punisher Shield reflects ${damageAmount} damage back to ${attacker.name}!`);
...
async function runBattle(interaction, player1Id, player2Id, battleType, npcTemplate = null) {
    ...
    player2 = {
        ...
        activeEffects: Array.isArray(stored.activeEffects) ? stored.activeEffects.slice() : [],
 ...
 // ...existing code...
    ...
    customBgUrl = npcData.background;
        ...
        // Calculate damage that would be dealt by each action
        const damageToPlayer2 = player1Action.damage || 0;
        const damageToPlayer1 = player2Action.damage || 0;

        // Apply damage with reflection checks
        applyDamageWithReflection(player1, player2, damageToPlayer2, player1Action);
        applyDamageWithReflection(player2, player1, damageToPlayer1, player2Action);
