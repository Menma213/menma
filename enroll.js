...
const createMovesEmbed = () => {
    ...
    // Button 2 - Transform (if enough chakra)
    if (player.chakra >= 5) {
        row.addComponents(
            new ButtonBuilder()
    ...
    return { embed: movesEmbed, components: [row] };
        ...
        const checkBattleStatus = async () => {
            ...
            player.chakra = Math.min(player.chakra + 2);
...
battleCollector.on('collect', async (i) => {
    ...
    if (i.customId === `attack-${userId}`) {
        ...
        player.chakra = Math.min(player.chakra + 1);
        enemyMove = processEnemyMove();
        player.health -= enemyMove.damage;
    ...
    if (battleStatus) {
        if (enemy.currentHealth <= 0) {
            const victoryEmbed = new EmbedBuilder()
                ...
                .setColor('#4B0082') // Using the purple color from earlier
                .setImage('https://static.wikia.nocookie.net/naruto/images/5/50/Team_Kakashi.png/revision/latest?cb=20161219035928') // Add your image URL here
                .setFooter({ 
