...
global.dataManager = dataManager; // Make it global for all commands
...
// API: Get Jutsus
webApp.get('/api/jutsus', (req, res) => {
    const jutsusPath = path.join(__dirname, 'menma', 'data', 'jutsus.json');
    if (fs.existsSync(jutsusPath)) {
        res.sendFile(jutsusPath);
    } else {
        res.status(404).json({ error: "Jutsus not found" });
    }
});

// API: Get User Stats
webApp.get('/api/user/:id', (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            bloodline: userData.bloodline || 'None',
            jutsus: userData.jutsu ? (Array.isArray(userData.jutsu) ? userData.jutsu.length : Object.keys(userData.jutsu).length) : 0
        });
    } catch (err) {
